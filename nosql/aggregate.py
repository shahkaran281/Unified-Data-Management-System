...
def aggregate_table(tableName, count_attribute, group_by , order_by = None , chunk_size = 100):
    ...
    try:
        ...
        for curr_json in data:
            for key , value in curr_json.items():
                ...
                if(group_by_val is not None and group_by_val not in distinct_counts):
                    distinct_counts[group_by_val] = [value.get(count_attribute , None)]
                elif(group_by_val is not None and group_by_val in distinct_counts):
                    distinct_counts[group_by_val].append(value.get(count_attribute , None))
        ...
        print( f"Error: File '{tableName}.json' not found in the 'data' directory.")

def process_chunk(dict , isLastChunk = False , order_by = None):
    if(isLastChunk):
        unique_values_dict = {name: len(set(values)) for name, values in dict.items()}
        if(order_by):
            unique_values_dict = {k: v for k, v in sorted(unique_values_dict.items(), key=lambda item: item[1])}
