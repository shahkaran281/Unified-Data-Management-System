        ...
        with open(csv_file, 'r', newline='') as file:
            ...
            for idx, row in enumerate(reader):
                if row[column_name] == target_value:
                    return row, idx
            # If the loop completes without finding the target value, return an error message
            return None, f"Target value '{target_value}' not found in column '{column_name}'."
...
def delete_table(tableName, uuid_to_del):
    ...
    print("UUID:", uuid)
    result, idx = load_row_by_value(
        f"data/{tableName}.csv", 'uuid', uuid_to_del)
    if result:
        print(result, idx)
        process = subprocess.Popen([f"scripts/update.sh", str(
            idx+2), f"data/{tableName}.csv"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        print(f"Error: {idx}")
  # Decode bytes to string for Python 3.x
...
if __name__ == "__main__":
    ...
    pattern = r'^from\s+([A-Za-z][A-Za-z0-9]*)\s+([A-Za-z0-9-]+)'
