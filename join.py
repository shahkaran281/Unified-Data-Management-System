...
def join_and_print_selected_columns(file1, file2, key1='uuid', key2='fk', columns1=[], columns2=[]):
    ...
    missing_columns1 = [col for col in columns1 if col not in data1.get(next(iter(data1), {}))]
    missing_columns2 = [col for col in columns2 if col not in data2.get(next(iter(data2), {}))]

    if missing_columns1:
        print(f"Columns {', '.join(missing_columns1)} not found in table {file1}.")
        return
    if missing_columns2:
        print(f"Columns {', '.join(missing_columns2)} not found in table {file2}.")
        return
    ...
    for key in data1:
        if key in data2:
            ...
            selected_data1 = [combined_data.get(column, '') for column in columns1]
            selected_data2 = [combined_data.get(column, '') for column in columns2]
...
if __name__ == '__main__':
    ...
    if match:
        ...
        join_and_print_selected_columns(table1, table2, col1, col2, columns1, columns2)
    else:
        print("No match found in the input string. PLEASE USE: JOIN [COLUMNS_FROM_TABLE1_SEPARATED_BY_COMMA], [COLUMNS_FROM_TABLE2_SEPARATED_BY_COMMA] FROM TABLE1.COL1 = TABLE2.COL2")
